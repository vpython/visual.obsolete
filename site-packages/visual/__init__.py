...
################################
# Gary Ruben <gruben@bigpond.net.au> says the following is 10% faster:
#MATHTYPES = (float, int, long)  # include once

#def func(x):
#    if type(x) in MATHTYPES: return _m_func(x)
#    return _n_func(x)
################################ 
...
def frexp(x):
    ...
    return _n_frexp(x)
def hypot(x,y):
    if set([type(x), type(y)]).issubset((float, int, long)):
        return _m_hypot(x,y)
    return _n_hypot(x,y)
def ldexp(x):
    t = type(x)
...
from visual.primitives import points, distant_light, local_light
