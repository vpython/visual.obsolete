...
class text(object):
    ...
    def __init__(self, pos=(0,0,0), axis=(1,0,0), up=(0,1,0), height=1, color=color.white, 
        ...
        if '\n' in text:
            raise ValueError, 'Currently it is not possible to include \\n in the text'
        if keywords.has_key('display'):
            self.__display = keywords['display']
            del keywords['display']
        else:
    ...
    def createText(self):
        try:
            ...
            try:
                realfont = findFont(self.__font)
            except:
                realfont = ''
            raise ValueError, 'Error in handling font '+self.__font+' ('+realfont+')'
        self.paintText()
    ...
    def getGlyphContour(self):
        ...
        for newchar in self.__text:
            ...
            ccontours = []
            for contour in self.contours:
                if len(contour) > 0: ccontours.append(contour)

            self.contours = ccontours
            ...
            for i in range(len(self.contours)):
                totlen = lenctr(self.contours[i])
                lengths.append((totlen,i))
                if totlen > maxl:
                    mli = i
                    maxl = totlen

            lc = len(self.contours)
            if lc >= 4:
                lengths.sort()
                lengths.reverse()
                ocontours = []
                for ll in lengths:
                    ocontours.append(self.contours[ll[1]])
                self.contours = ocontours
            ...
            if (mli > 0 and newchar != "%"):
                ...
                if (indxf == -1 and indxc == -1):
                    #print "changing contour"
                    maxc = self.contours.pop(mli)
                    self.contours.insert(0, maxc)
    ...
    def set_height(self, height):
        ...
        self.back.pos *= factor
        self.front.pos *= factor
        self.sides.pos[...,0] *= factor
        self.sides.pos[...,1] *= factor
        self.__width *= factor
