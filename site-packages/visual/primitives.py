...
def process_init_args_from_keyword_dictionary( displayobject, keywords):
   ...
   # pos if color is a single tuple. But if color is a list, must set up
   # pos before assigning colors.
   ...
   if keywords.has_key('color'):
      ...
      try:
         ...
         if colors_length == 3:
            ...
            setcolors = False # if colors is a sequence of multiple colors
      except:
         displayobject.color = colors # single scalar value
   ...
   if not setcolors: # sequence of colors
      if len(colors) == len(displayobject.pos):
         for n in range(len(displayobject.pos)):
            ...
            elif len(displayobject.color[n]) == 4 and len(colors[n]) == 3:
               for i in range(3):
                  displayobject.color[n][i] = colors[n][i]
            else:
               raise RuntimeError, "Mismatch of pos (%i components) and color (%i components)" % \
               (len(displayobject.pos[n]), len(colors[n]))
      else:
         raise RunTimeErrorj, "Length of pos (%i) and color (%i) sequences are not equal" % \
...
class points( cvisual.points, py_renderable):
   ...
   opacity = property( get_opacity, cvisual.points.set_opacity, None)
   alpha = property( get_opacity, cvisual.points.set_opacity, None)
...
class helix(object):
   def __init__( self, _other=None, pos=vector(),
         axis=vector(1,0,0), radius=0.2, length=None,
        coils=5, thickness=None, color=color.white, **extrakwds):
    axis = vector(axis)
    if length is None:
        length = axis.mag
    self.cached_length = length
    if ('frame' in extrakwds):
       try:
          self.frame = frame(frame=extrakwds['frame'], pos=pos, axis=length*axis.norm())
       except:
          raise RuntimeError, "Do not create helix with nonuniform axes."
       del extrakwds['frame']
    else:
       try:
          self.frame = frame(pos=pos, axis=length*axis.norm())
       except:
          raise RuntimeError, "Do not create helix with nonuniform axes."
    ...
    x_col = arange(0, self.cached_length+dx, dx)
    pos_data = zeros((len(x_col),3), float64)
    pos_data[:,0] = arange(0, self.cached_length+dx, dx)
    pos_data[:,1] = radius * sin(k*pos_data[:,0])
    pos_data[:,2] = radius * cos(k*pos_data[:,0])
    self.helix = curve( frame = self.frame, radius = thickness/2.,
        color = color, pos=pos_data, **extrakwds)
   ...
   def get_z(self):
    return self.frame.pos.z

   def set_axis(self, axis):
    self.frame.axis = vector(axis)
    self.frame.scale.x = vector(axis).mag / self.cached_length
   ...
   def set_length(self, length):
    ...
    self.cached_length = length
    self.frame.axis = length*self.frame.axis.norm()
