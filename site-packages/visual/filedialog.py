...
def save_file(file_extensions=None, x=100, y=100, title="Open", mode='w', maxfiles=20):
...
def filedialog(file_extensions=None, x=100, y=100, title="Open", mode='rU', maxfiles=20):
    ...
    while menus.visible:
        ...
        if need_to_scroll:
            Nfiles = maxfiles
            hscroll = (ytopscroll-ybottomscroll)*maxfiles/(len(files)-1)
            if hscroll < hmenu:
                hscroll = hmenu
            dy = (ytopscroll-ybottomscroll-hscroll)/(len(files)-1-maxfiles)
        ...
        while menus.visible:
            ...
            if menus.mouse.events:
                ...
                if drag and m.release == 'left':
                    ...
                    scroll.offset = scroll.y-m.pos.y
                    drag = True
                elif m.click == 'left':
            ...
            if drag:
                if scroll.y+0.5*hscroll == ytopscroll:
                    if scroll.offset < -0.5*hmenu:
                        scroll.offset = scroll.y-mpos.y
                    else:
                        scroll.y = mpos.y+scroll.offset
                elif scroll.y-0.5*hscroll == ybottomscroll:
                    if scroll.offset > 0.5*hmenu:
                        scroll.offset = scroll.y-mpos.y
                    else:
                        scroll.y = mpos.y+scroll.offset
                ...
                if newtopmenu != topmenu:
                    if newtopmenu > len(files)-1-maxfiles:
                        newtopmenu = len(files)-1-maxfiles
                    topmenu = newtopmenu
                    select.visible = 0
                    for n, lab in enumerate(labels[1:]):
                        lab.text = files[n+1+topmenu][0]
                        if files[n+1+topmenu][2]:
                            select.y = ytop-(n+1)*hmenu
                            select.visible = 1
                        elif files[n+1+topmenu][1]:
                            lab.color = dircolor
                        else:
                            lab.color = filecolor
            else:
