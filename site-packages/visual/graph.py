...
minresize = 1.0 # need to expand by this much before resizing (1.0 -> always resize)
...
dz = 1 # offset for plots relative to axes and labels
xaxis = 0
yaxis = 1
negaxis = 0
posaxis = 1
...
class gdisplay:
    def __init__(self, x=0, y=0, width=800, height=400,
        ...
        self.display.range = (1.1*self.width/2,1.1*self.height/2,dz)
        self.autoscale = [1, 1]
        ...
        if ytitle is not None:
            ...
            self.ytitlewidth = 0
        self.graphs = [] # list of gcurves, gvbars, ghbars, gdots, ghistogram, and axes
        self.labels = [] # list of titles, minor and major tick marks
                ...
                for nn in range(maxminorticks):
                    if axis == xaxis:
                        ...
                        self.minorticks[axis][axissign].append(label(display=self.display, xoffset=-tminor,
                            linecolor=grey, visible=0, box=0, opacity=0))
                    self.labels.append(self.minorticks[axis][axissign][-1])
                ...
                for nn in range(maxmajorticks):
                    if axis == xaxis:
                        ...
                        self.majorticks[axis][axissign].append(label(display=self.display, xoffset=-tmajor, 
                            ...
                            linecolor=grey, visible=0, box=0, opacity=0))
                    self.labels.append(self.majorticks[axis][axissign][-1])
    ...
    def setcenter(self):
        ...
        x = (xright-xleft)/2
        y = (ytop-ybottom)/2
        self.display.center = self.gscale*array([x,y,0])
    ...
    def axisdisplay(self, axis, axissign):
        ...
        if self.axis[axis][axissign] is None: # new; no axis displayed up till now
            ...
            self.axis[axis][axissign] = curve(pos=axispos*self.gscale, color=grey, display=self.display)
            self.graphs.append(self.axis[axis][axissign])
            if (self.title[axis][0] is not None) and (not self.title[axis][1]):
                title = self.title[axis][0]
                self.title[axis][0] = label(display=self.display, pos=titlepos*self.gscale, text=title,
                            font=graphfont, xoffset=tminor, opacity=0, box=0, line=0)                    
                self.title[axis][1] = 1
                self.labels.append(self.title[axis][0])
            ...
            for x1 in newmajormarks:
                ...
                obj.visible = 1
                if axis == xaxis:
                    obj.pos = array([sign*x1,y0,0])*self.gscale
                    obj.yoffset = -tmajor 
                else:
                    obj.pos = array([x0,sign*x1,0])*self.gscale
                    obj.xoffset = -tmajor
                nmajor = nmajor+1
            ...
            for x2 in arange(0.+d2, self.minmax[axis][axissign]+1.01*d2, d2):
                ...
                obj.visible = 1
                if axis == xaxis:
                    obj.pos = array([sign*x2,y0,0])*self.gscale
                else:
                    obj.pos = array([x0,sign*x2,0])*self.gscale
                nminor = nminor+1
            ...
            if axis == xaxis:
                self.axis[axis][axissign].pos = array([[0,0,0],[sign*self.minmax[axis][axissign], y0, 0]])*self.gscale
            else:
                self.axis[axis][axissign].pos = array([[0,0,0],[x0,sign*self.minmax[axis][axissign],0]])*self.gscale
            ...
            if self.title[axis][1] and axissign == posaxis:
                ...
                self.title[axis][0].pos = titlepos*self.gscale
            ...
            if needminor: # adjust minor tick marks
                ...
                for x2 in arange(0.+d2, self.minmax[axis][axissign]+1.01*d2, d2):
                    ...
                    if (d2 <> oldd2) or x2 > self.lastlabel[axis][axissign]-0.01*d2:
                        if axis == xaxis:
                            self.minorticks[axis][axissign][nminor].pos = array([sign*x2,y0,0])*self.gscale
                        else:
                            self.minorticks[axis][axissign][nminor].pos = array([x0,sign*x2,0])*self.gscale
        ...
        if self.autoscale[yaxis]:
            if y > self.lastminmax[yaxis][posaxis]:
                ...
                if (self.lastminmax[yaxis][posaxis] == 0 or
                        (y >= minresize*self.lastminmax[yaxis][posaxis])):
                    redoy = 1
            elif -y > self.lastminmax[yaxis][negaxis]:
                self.minmax[yaxis][negaxis] = -y
                if (self.lastminmax[yaxis][negaxis] == 0 or
                        (-y >= minresize*self.lastminmax[yaxis][negaxis])):
                    redoy = 1

        if (redox or redoy):
            newgscale = self.calculategscale()
            factor = newgscale/self.gscale
            for obj in self.graphs:
                obj.pos *= factor
            for obj in self.labels:
                obj.pos = array(obj.pos)*factor
            self.gscale = newgscale
            if redox:
                for axissign in range(2):
...
class gcurve:
    ...
    def plot(self, **args):
        ...
        self.gcurve.append(pos=(pos[0]*self.gdisplay.gscale[0],pos[1]*self.gdisplay.gscale[1],dz), color=self.color)
...
class gdots:
    ...
    def plot(self, **args):
        pos = primitiveargs(self,args)
        self.gdisplay.resize(pos[0], pos[1])
        self.dots.append((pos[0]*self.gdisplay.gscale[0],pos[1]*self.gdisplay.gscale[1],dz))
...
class gvbars:
    ...
    def plot(self, **args):
        ...
        for pt in self.makevbar(pos):
            self.vbars.append(pos=(pt[0]*self.gdisplay.gscale[0],pt[1]*self.gdisplay.gscale[1],-dz), normal=znormal)
...
class ghbars:
    ...
    def plot(self, **args):
        ...
        for pt in self.makehbar(pos):
            self.hbars.append(pos=(pt[0]*self.gdisplay.gscale[0],pt[1]*self.gdisplay.gscale[1],-dz), normal=znormal)
    ...
    def plot(self, data=None, accumulate=None, average=None, color=None):
        ...
        for nbin in range(len(self.bins)):
            ...
            if self.nhist == 1.:
                ...
                self.vbars.pos[6*(nbin+1)+2][1] = pos[1]*self.gdisplay.gscale[1]
                self.vbars.pos[6*(nbin+1)+3][1] = pos[1]*self.gdisplay.gscale[1]
                self.vbars.pos[6*(nbin+1)+5][1] = pos[1]*self.gdisplay.gscale[1]
