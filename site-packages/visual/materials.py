...
Stable Interfaces:
    ...
    materials.texture( channels, data, mapping, data_type=None, mipmap=True,
                       interpolate=True, name="texture" )
        Create a material by "extruding"/projecting a 2D image.
        See VPython documentation for details.

    materials.loadBTX( filename, (x,y,bands) )
        Load an image from a BTX (raw 8 bit) file.  The shape must
        be known in advance, since BTX files have no header.  This is useful
        for supplying the data parameter to materials.texture() when the
        program might be run without PIL or a similar library available.
    
    materials.saveBTX( filename, image )
        Save a PIL image or numpy array into a BTX file.
...
def shader( name, shader, version, library = library, **kwargs ):
    ...
    if max_version < 4.00 or min_version >= 4.10:
...
materials = [
    shader( name = "unshaded", version = (1.00,4.00), library = "", shader = """
[vertex]
        void main() {
...
[fragment]
    ...
    shader( name = "plastic", version = 4.00, shader = """
        ...
        """),
    shader( name = "rough", version = 4.00, textures = [ tx_turb3 ], shader = """
        [fragment]
        ...
        """),
    shader( name = "wood", version = 4.00, textures = [tx_wood, tx_turb3], shader = """
        [fragment]
        ...
        """),
    shader( name = "marble", version = 4.00, textures=[tx_turb3], shader = """
        [fragment]
        uniform sampler3D tex0;
    ...
    texture( name = "earth",
        ...
        mapping = "spherical",
        interpolate = True ),
    shader( name = "show_mat_pos", version = 4.00, textures = [], shader = """
        [fragment]
        void material_main() {
