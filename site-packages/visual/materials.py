...
import cvisual
from numpy import array, reshape, fromstring, uint8, ndarray, zeros, asarray
import os.path
...
def convert_data(data):
    if hasattr(data, "getbands"): # For convenience, accept PIL images
        data = reshape( array( data.getdata(), uint8 ),
                        (data.size[0],data.size[1],len(data.getbands())) )
    elif isinstance( data, ndarray ):
        if data.dtype != uint8:
            data = array( 255*data, uint8 )
    else:
        data = array( 255*array(data), uint8 )
    return data
...
def saveTGA(fileid, data):
    ...
    output = zeros(Lheader+length, uint8)
...
def loadTGA(fileid):
    ...
    data = fromstring( fileid.read(), uint8 )
...
def texture(data, channels=None, 
    ...
    data = convert_data(data) # make float or utype array
