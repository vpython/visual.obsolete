...
[fragment]
    ...
    vec3 lightAt( vec3 N, vec3 E, vec3 diffuse, vec3 specular, float shininess ) {
        ...
        if (count <= 2) {
            for(int i=0; i<2; i++) {
                if (i<count) {
                    vec3 L = normalize( light_pos[i].xyz - v*light_pos[i].w );
                    color += (light_color[i].rgb * max(dot(N,L), 0.0))*diffuse;
                    if (shininess != 0.0) {
                        vec3 R = -reflect(L,N);
                        color += specular * light_color[i].rgb * pow(max(dot(R,E),0.0),shininess);
                    }
                }
            }
        } else if (count <= 4) {
    ...
    vec3 light() {
        return lightAt( normalize(N), normalize(-v), gl_Color.rgb, vec3(.8,.8,.8), 64.0 );
    }
"""
...
materials = [
    shader( name = "unshaded", library = "", shader = """
[vertex]
        void main() {
...
[fragment]
    ...
    shader( name = "diffuse", shader = """
        [fragment]
        void main() {
            gl_FragColor.rgb = lightAt( N, normalize(-v), gl_Color.rgb, vec3(0,0,0), 0.0 );
    ...
    shader( name = "rough", textures = [ tx_turb3 ], shader = """
        ...
        void main() {
            ...
            gl_FragColor.rgb = lightAt( normalize(N2), normalize(-v),
                                ...
                                - gl_Color.rgb * gl_LightModel.ambient.rgb * .7;
    ...
    shader( name = "wood", textures = [tx_wood, tx_turb3], shader = """
        ...
        void main() {
            ...
            vec2 wt = mat_pos.yz*.85 + mat_pos.x*.1 + T.xy * .05;
    ...
    shader( name = "marble", textures=[tx_turb3], shader = """
        ...
        void main() {
            ...
            vec3 N2 = N + T*1.;
            gl_FragColor.rgb = lightAt( normalize(N2), normalize(-v), C*gl_Color.rgb, vec3(.8,.8,.8), 100. );
            gl_FragColor.a = gl_Color.a;
        }
        """),
    ...
    texture( name = "earth",
        ...
        mapping = "spherical",
        interpolate = True ),
    shader( name = "show_mat_pos", textures = [], shader = """
        [fragment]
        void main() {
            gl_FragColor.rgb = mat_pos * .5;
            if ( fract( mat_pos.x * 10. + .021 ) < .02 ) gl_FragColor.rgb *= 0.5;
            if ( fract( mat_pos.y * 10. + .021 ) < .02 ) gl_FragColor.rgb *= 0.5;
            if ( fract( mat_pos.z * 10. + .021 ) < .02 ) gl_FragColor.rgb *= 0.5;
            if ( mat_pos.x < 0. || mat_pos.x > 1. ) gl_FragColor.rgb = vec3(1.,0.8,0.8);
            if ( mat_pos.y < 0. || mat_pos.y > 1. ) gl_FragColor.rgb = vec3(0.8,1.,0.8);
            if ( mat_pos.z < 0. || mat_pos.z > 1. ) gl_FragColor.rgb = vec3(0.8,0.8,1.);
            gl_FragColor.a = gl_Color.a;
        }
        """ ),
]
