...
[fragment]
    ...
    material( name = "rawturbulence", textures = [ tx_turb3 ], shader = shader( """
        ...
        void main() {
            vec3 tp = mat_pos.xyz;
            vec3 T = texture3D(tex, tp).rgb;
            T += texture3D(tex, tp * 8.).rgb/8.;
            gl_FragColor.rgb = T.xyz;
            gl_FragColor.a = 1.0;
        }
        """ )),
    ...
    material( name = "wood1", textures = [tx_wood], shader = shader( """
        ...
        void main() {
            vec2 wt = mat_pos.xy*.9;
            vec3 C = texture2D( wood, wt ).rgb;
    ...
    material( name = "metal", color = (1,.95,.7), textures = txs_env, shader = shader("""
        ...
        void main() {
            ...
            vec3 wN = normalize( vec3( camera_world * vec4(gl_NormalMatrix * gl_Normal, 0.0) ) );
        ...
        void main() {
            ...
            if (vR.z>0.0) {
                uv = (vR.xy / (2.*(1.+vR.z))) + .5;
                uv.y = 1.-uv.y;
                C = texture2D( tex0, uv ).rgb;
            } else {
                uv = (vR.xy / (2.*(1.-vR.z))) + .5;
                uv.x = 1.-uv.x;
                uv.y = 1.-uv.y;
                C = texture2D( tex1, uv ).rgb;
            }
    ...
    material( name = "metal2", color = (1,.95,.7), textures = txs_env, shader = shader("""
        ...
        void main() {
            ...
            vec4 wP = camera_world * (gl_ModelViewMatrix * gl_Vertex);
            wN = vec3( camera_world * vec4(gl_NormalMatrix * gl_Normal, 0.0) );
...
##    material( name = "borken", textures = [], shader = shader( """
##        [fragment]
##        void main() {
##            -
##        }
