...
def texture(data, channels=None, data_type=None,
            mapping="rectangular", mipmap=True, interpolate=True, name="texture"):

    # TODO: Is this now irrelevant, now that we're using targa files?
...
##                        (data.size[0],data.size[1],len(data.getbands())) )
    ...
    if data_type is None:
        if not isinstance( data, ndarray ):
            data = array(data, float) # assume float if data_type not given
##            raise TypeError("If data_type is not defined, data must be a numpy array.")
            ...
            # If data_type is None, could be an array containing red as (1,0,0) etc.,
            # which would yield "i" through the array-creation machinery (unlike "u"):
            if data.dtype.kind == "i":
                data = array(data, float)
            
    else:
        # TODO: restrict data_type?
        # If user has 0-255 data, they MUST specify data_type as int or utype.
        # Conversely, if they specigy data_type as int or utype, data must be 0-255.
        if data_type is int: 
            data_type = ubyte # without this, 255 is way out of bounds
