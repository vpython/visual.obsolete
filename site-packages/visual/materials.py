...
Stable Interfaces:
    ...
    materials.texture( filename=None, data=None, mipmap=True,
                       interpolate=True, name="texture" )
        Create a material by "extruding"/projecting a 2D image.
        If name is given but not data, read targa file.
        If both name and data are given, save to targa file.
        If only data is given, texture is not saved to file.
        The data must be an array whose elements range 0-1,
        with 1, 2, 3, or 4 bytes per pixel.
...
import cvisual
from numpy import array, reshape, fromstring, ubyte, ndarray, zeros
import os.path
...
def get_image_data(filename):
    if filename[-4:] != ".tga":
        filename += ".tga"
    fd = open(filename, "rb")
    data = fromstring( open(filename,"rb").read(), ubyte )
    targa_type = data[2]
    ...
    if 3 <= bytes <= 4:
        red = image[0::bytes]+0 # make copy
        image[0::bytes] = image[2::bytes] # blue
        image[2::bytes] = red
        image = image.reshape((width,height,bytes))
    return (image,bytes)

texturePath = os.path.split( __file__ )[0] + "/"
data = get_image_data(texturePath+"turbulence3")[0] # the targa file is 512*512*3
tx_turb3 = raw_texture( data=reshape(data,(64,64,64,3)), interpolate=True, mipmap=False )
tx_wood = raw_texture( data=get_image_data(texturePath+"wood")[0], interpolate=True)

def get_image(filename, data, luminance):
    if filename is not None:
        image,bytes = get_image_data(filename)
    else:
        dims = data.shape
        width = dims[0]
        height = dims[1]
        if len(dims) < 3:
            bytes = 1
            image = reshape(array(255*data, ubyte), width*height)
        else:
            bytes = dims[2]
            image = reshape(array(255*data, ubyte), (width,height,bytes))
    if bytes == 1:
        if luminance:
            channels = ["luminance"]
        else:
        ...
        raise TypeError("Cannot have 2 bytes per pixel.")
    elif bytes == 3:
        channels = ["red", "green", "blue"]
    elif bytes == 4:
        channels = ["red", "green", "blue", "opacity"]
    else:
        raise TypeError("Must have 1, 3, or 4 values per pixel.")
    return (image, channels)

def save_to_targa(filename, data):
    dims = data.shape
    width = dims[0]
    height = dims[1]
    if len(dims) < 3:
    ...
    targa_type = 2 # 3 (rgb) or 4 (rgbo) bytes per pixel
    databytes = 255*data.flatten()
    if bytes == 1:
        targa_type = 3 # 1 byte per pixel
    elif bytes == 2: # luminance+opacity -> rgbo, (lum,lum,lum,opacity)
        bytes = 4
        length *= 2
        newdata = zeros(length,ubyte)
        luminance = databytes[0::2]
        newdata[0::4] = luminance
        newdata[1::4] = luminance
        newdata[2::4] = luminance
        newdata[3::4] = databytes[1::2] # opacity
        databytes = newdata
    elif 3 <= bytes <= 4: # rgb
        if bytes == 4:
            attributes += 3 # signal presence of opacity
        red = databytes[0::bytes]+0 # make copy
...
def texture(filename=None, data=None, luminance=True,
            mapping="rectangular", mipmap=True, interpolate=True, name="texture"):
    if filename is None and data is None:
        raise TypeError("Must specify either filename or data.")
    if data is not None:
        if filename is not None:
            save_to_targa(filename, data)
        data, channels = get_image(None, data, luminance)
    else: # filename only
        data, channels = get_image(filename, None, luminance)
        ...
        return shader(
            ...
            shader = """
                ...
                void material_main(void) {
                    vec4 C = texture2D( tex0, mat_pos.zy*vec2(-1,-1) );
        ...
        return shader(
            ...
            shader = """
                ...
                void material_main(void) {
                    vec4 C;
                    if ( front_face > 0.5 ) {
                        C = texture2D( tex0, mat_pos.zy*vec2(-1,-1) );
...
[fragment]
    ...
    shader( name = "wood", version = 5.00, textures = [tx_wood, tx_turb3], shader = """
        ...
        void material_main() {
            ...
            vec2 wood_pos = .85 * mat_pos.yz +    //< simple rectangular mapping
    ...
    texture( name="earth", filename = texturePath+"earth",
        mapping = "spherical"),
