...
class display( cvisual.display):
    def __init__( self, **keywords):
        ...
        if 'lights' not in keywords:
            distant_light( direction=(0.22, 0.44, 0.88), color=0.8, display=self )
            distant_light( direction=(-0.88, -0.22, -.44), color=0.3, display=self )
        self.select()
    def select(self):
    ...
    def _set_lights(self, n_lights):
        ...
        for lt in n_lights:
            if isinstance( lt, cvisual.light ):  #< TODO: should this be allowed?
                ...
                distant_light( direction=cvisual.vector(lt).norm(), color=cvisual.vector(lt).mag, display=self )
