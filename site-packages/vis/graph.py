...
# minmax[xaxis][negaxis], minmax[yaxis][negaxis] are minimum negative values;
# minmax[xaxis][posaxis], minmax[yaxis][posaxis] are maximum positive values.
...
class gdisplay:
    def __init__(self, x=0, y=0, width=800, height=400,
        ...
        if xmax is not None:
            if xmax > 0:
                ...
                if xmax < 0: raise ValueError("log of xmax must be positive")
        ...
        if xmin is not None:
            if xmin < 0:
                if self.logx: raise ValueError("For a log scale, 0 < xmin <= 1")
                self.minmax[xaxis][negaxis] = xmin
                self.autoscale[xaxis] = False
            else:
                if self.logx:
                    if not (0 < xmin <= 1): raise ValueError("For a log scale, 0 < xmin <= 1")
                    else: self.minmax[xaxis][negaxis] = log10(xmin)
                    self.autoscale[xaxis] = False
                else:
        ...
        if ymax is not None:
            if ymax > 0:
                ...
                if ymax < 0: raise ValueError("log of ymax must be positive")
        ...
        if ymin is not None:
            if ymin < 0:
                if self.logy: raise ValueError("For a log scale, 0 < ymin <= 1")
                self.minmax[yaxis][negaxis] = ymin
                self.autoscale[yaxis] = False
            else:
                if self.logy:
                    if not (0 < ymin <= 1): raise ValueError("For a log scale, 0 < ymin <= 1")
                    else: self.minmax[yaxis][negaxis] = log10(ymin)
                    self.autoscale[yaxis] = False
                else:
    ...
    def setcenter(self):
        ...
        xleft = self.minmax[xaxis][negaxis] # <= 0
        ytop = self.minmax[yaxis][posaxis]
        ybottom = self.minmax[yaxis][negaxis] # <= 0
        if (xleft==0):
    ...
    def setminorticks(self, axis, axissign, loglabel, dmajor, dminor):
        ...
        ## Since we have only 4 minor tick marks between major tick marks,
        ## if dmajor > 5, don't show any minor tick marks.
        if loglabel and (dmajor > 5): return 0
        ...
        while True:
            ...
            if loglabel: # have already excluded dmajor > 5
                if dmajor == 1:
                    if dminor > 0:
    ...
    def axisdisplay(self, axis, axissign):
        ...
        if self.axis[axis][axissign] is None: # new; no axis displayed up till now
            if axis == xaxis:
                axispos = array([(0,y0,0), (self.minmax[axis][axissign],y0,0)])
                titlepos = array([self.minmax[axis][posaxis],y0,0])
            else:
                axispos = array([(x0,0,0), (x0,self.minmax[axis][axissign],0)])          
            ...
            if axis == xaxis:
                self.axis[axis][axissign].pos = array([[0,0,0],[self.minmax[axis][axissign], y0, 0]])
            else:
                self.axis[axis][axissign].pos = array([[0,0,0],[x0,self.minmax[axis][axissign],0]])
        ...
        if self.autoscale[xaxis]:
            if x > self.lastminmax[xaxis][posaxis]:
                ...
                if (self.lastminmax[xaxis][negaxis] == 0 or
                    ...
                    redox = True
        elif self.axis[xaxis][posaxis] is None or self.axis[xaxis][negaxis] is None:
            redox = True
        ...
        if self.autoscale[yaxis]:
            if y > self.lastminmax[yaxis][posaxis]:
                ...
                if (self.lastminmax[yaxis][negaxis] == 0 or
                    ...
                    redoy = True
        elif self.axis[yaxis][posaxis] is None or self.axis[yaxis][negaxis] is None:
            redoy = True
