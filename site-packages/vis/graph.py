...
class gdisplay:
    ...
    def axisdisplay(self, axis, axissign):
        ...
        if self.axis[axis][axissign] is None: # new; no axis displayed up till now
            ...
            nminor = nmajor = nlog = 0
            for x2 in arange(0.+d2, self.minmax[axis][axissign]+1.01*d2, d2):
                if not loglabel and x2 > self.minmax[axis][axissign]: break
                if x2 % d1 < d2/2.:
                    nmajor += 1
                    nlog += 1
                    continue # don't put minor tick where there is a major one
                if loglabel and d1*(nmajor+logticks[nlog%5]) > self.minmax[axis][axissign]: break
                obj = self.minorticks[axis][axissign][nminor]
                obj.visible = True
                if axis == xaxis:
                    if self.logx:
                        obj.pos = array([sign*d1*(nmajor+logticks[nlog%5]),y0,0])
                    else:
                        obj.pos = array([sign*x2,y0,0])
                else:
                    if self.logy:
                        obj.pos = array([x0,sign*d1*(nmajor+logticks[nlog%5]),0])
                    else:
            ...
            if needminor: # adjust minor tick marks
                ...
                for x2 in arange(0.+d2, self.minmax[axis][axissign]+1.01*d2, d2):
                    ...
                    if (d2 != oldd2) or x2 > self.lastlabel[axis][axissign]-0.01*d2:
                        if axis == xaxis:
                            if self.logx:
                                self.minorticks[axis][axissign][nminor].pos = \
                                        array([sign*d1*(nmajor+logticks[nlog%5]),y0,0])
                            else:
                                self.minorticks[axis][axissign][nminor].pos = array([sign*x2,y0,0])
                        else:
                            if self.logy:
                                self.minorticks[axis][axissign][nminor].pos = \
                                        array([x0,sign*d1*(nmajor+logticks[nlog%5]),0])
                            else:
                                self.minorticks[axis][axissign][nminor].pos = array([x0,sign*x2,0])
                        self.minorticks[axis][axissign][nminor].visible = True
                    nminor += 1
                    nlog += 1
...
def primitiveargs(obj,arguments):
        ...
        if 'pos' in arguments:
            x,y = arguments['pos']
            try:
                if obj.gdisplay.logx: x = log10(x)
                if obj.gdisplay.logy: y = log10(y)
            except:
                raise ValueError("Cannot take log of negative number: (x,y) = ({0:0.3G},{1:0.3G})".format(float(x),float(y)))
            return ((x,y),c)
        else:
    ...
    def __init__(self, dot=False, dot_color=None, size=8, shape='round', **args):
        ...
        if pos is not None:
            if not logx and not logy:
                for xy in pos: self.plot(pos=xy)
            elif logx and not logy:
                for xy in pos: self.plot(pos=(log(xy[0]),xy[1]))
            elif logy and not logx:
                for xy in pos: self.plot(pos=(xy[0],log(xy[1])))
            else:
                for xy in pos: self.plot(pos=(log(xy[0]),log(xy[1])))

    def plot(self, **args):
        pos,c = primitiveargs(self,args)
        self.gdisplay.resize(pos[0],pos[1])
        self.gcurve.append(pos=(pos[0],pos[1],2*dz), color=c)
        if self.dot:
