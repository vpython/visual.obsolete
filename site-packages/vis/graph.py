...
class gdisplay:
    def __init__(self, x=0, y=0, width=800, height=400,
        ...
        if xmin is not None:
            if xmin < 0:
                if self.logx: raise ValueError("For a log scale, xmin must be positive")
        ...
        if ymin is not None:
            if ymin < 0:
                if self.logy: raise ValueError("For a log scale, ymin must be positive")
        ...
        if self.autoscale[xaxis]:
            if x > self.lastminmax[xaxis][posaxis]:
                ...
                if (self.lastminmax[xaxis][negaxis] == 0 or
                    ...
                    redox = True
        elif self.axis[xaxis][posaxis] is None and self.axis[xaxis][negaxis] is None:
            redox = True
        ...
        if self.autoscale[yaxis]:
            if y > self.lastminmax[yaxis][posaxis]:
                ...
                if (self.lastminmax[yaxis][negaxis] == 0 or
                    ...
                    redoy = True
        elif self.axis[yaxis][posaxis] is None and self.axis[yaxis][negaxis] is None:
            redoy = True
