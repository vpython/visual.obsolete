...
def convert(pos=(0,0,0), up=(0,1,0), up0=(0,1,0), points=None, closed=True):
        pos = vector(pos)
        up = norm(vector(up))
        up0 = norm(vector(up0))
...
class star():
    def __init__(self, pos=(0,0,0), radius=3, np=5, iradius=None, rotate=0.0, thickness=None,
                  roundness=0.0, invert=False, scale=1.0, xscale=1.0, yscale=1.0, up=(0,1,0)):
        if thickness is not None:
            raise AttributeError("Thickness is not allowed in a star path")
        c = sh.star(np=np, radius=radius, iradius=iradius, rotate=rotate,
                  roundness=roundness, invert=invert, scale=scale, xscale=xscale, yscale=yscale)
        self.pos = convert(pos=pos, up=up, points=c.contour(0))
        self.up = up
