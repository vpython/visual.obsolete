...
class extrusion (py_renderable_uniform, py_renderable_arrayobject, cvisual.extrusion ):
    ...
    def make_shapedata(self, datalist):
        ncontours = len(datalist)
        pointers = zeros((ncontours+1,2),int)
        pointers[0] = (ncontours,0)
        datalength = 0
    ...
    def set_shape(self, shape):
        ...
        for i in range(len(shape)):
            ...
            if (shape.isHole(i) and (shape.orientation(i) == -1) or
                ...
                datalist.append(shape.contour(i))
        contours, pcontours = self.make_shapedata(datalist)
        strips, pstrips = self.make_shapedata(shape.triStrip())
        # Send pointers, contour data, and shape.triStrip data to Visual
        self.set_contours(contours, pcontours, strips, pstrips)
        
    pos = property( cvisual.extrusion.get_pos, cvisual.extrusion.set_pos, None)
    ...
    shape = property( None, set_shape, None)
