...
class faces( py_renderable_arrayobject, cvisual.faces ):
    ...
    def set_source(self, source):
        if isinstance(source, extrusion) or isinstance(source, text):
            if isinstance(source, text): source = source.extrusion
            source.visible = False
            data = source.faces_render()
            del source
            L = len(data)/3
            self.pos = data[:L]
            self.normal = data[L:2*L]
            self.color = data[2*L:]
        else:
            raise AttributeError("A face's source must be an extrusion object.")

    def get_source(self):
        raise AttributeError("A face's source is not readable.")
