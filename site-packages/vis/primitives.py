...
from numpy import array, zeros, arange, float64, sin, cos
from . import crayola
color = crayola
from math import pi
...
class extrusion (py_renderable_uniform, py_renderable_arrayobject, cvisual.extrusion ):
    ...
    def make_shapedata(self, datalist, closed):
        ...
        pointers = zeros((ncontours+1,2),int)
        ...
        for i, c in enumerate(datalist):
            ...
            datalength += length
        contours = zeros((datalength,2),float)
        for i, c in enumerate(datalist):
        ...
        try:
            for i in range(len(shape)):
                ...
                if s[-1] == s[0]: # discard final point if same as initial
                    s = s[:-1]
                # Require hole contours to run CCW (1), external contours CW (-1)
                if (shape.isHole(i) and (shape.orientation(i) == -1) or
                    (not shape.isHole(i)) and shape.orientation(i) == 1):
            ...
            for i in range(len(shape)-1):
                ...
                x2 = s[i+1][0]
                y2 = s[i+1][1]
                curl += x1*(y2-y1) - y1*(x2-x1)
            if curl > 0: # Need to reverse the order of points in this contour
                s.reverse()
            if shape[-1] == shape[0]: # discard final point if same as initial, mark closed
                s = s[:-1]
                closed = True
            else:
                closed = False
...
class text(object):
    ...
    def paintText(self):
        ...
        shape = shapes.text(text=self.__text, font=self.__font,
                        align=self.__align, height=self.__height)
        ...
        self.__upper_left = shape.upper_left
        self.__upper_right = shape.upper_right
        self.__lower_left = shape.lower_left
        self.__lower_right = shape.lower_right

        dy = self.__height-(shape.upper_left.y-shape.lower_left.y)/2
        if self.__align == 'left':
            displace = vector(shape.width/2,dy,0)
            dstart = vector(-shape.width/2,0,0)
        elif self.__align == 'right':
            displace = vector(-shape.width/2,dy,0)
            dstart = vector(shape.width/2,0,0)
        else:
            displace = vector(0,dy,0)
            dstart = vector(0,0,0)
        self.__frame.pos = displace
        self.__start = shape.start+dstart
        for line in range(len(shape.starts)):
            shape.starts[line] += dstart
        self.__starts = shape.starts

        if self.__depth.z < 0: # adjust for case of backward extrusion
            p = [vector(0,0,0), self.__depth]
        else:
            p = [self.__depth, vector(0,0,0)]
        self.extrusion = extrusion(frame=self.__frame, pos=p, shape=shape, 
                  color=self.__color, material=self.__material)
    ...
    def get_starts(self):
        ...
        for line in range(len(self.lines)):
            s.append(self.__frame.frame_to_world(self.__starts[line]))
        return s
    ...
    def set_depth(self, depth):
        ...
        try:
            self.extrusion # need an extrusion to exist to be able to change its depth
            if self.__depth.z < 0:
                self.extrustion.pos = [self.__depth, (0,0,0)]
            else:
                self.extrustion.pos = [(0,0,0), self.__depth]
        except:
            self.paintText()
    def get_depth(self):
        return self.__depth
