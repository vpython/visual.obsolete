...
def text(pos=(0,0), text="", font=None, height=1.0, align='left', spacing=0.03,
    ...
    shape.shift(0,dy.y)
    shape.upper_left = upper_left+dy
    shape.upper_right = upper_right+dy
    shape.lower_left = lower_left+dy
    shape.lower_right = lower_right+dy
    if align == 'left':
        x = 0
    elif align == 'right':
        x = -width
        ...
        x = -0.5*width
    shape.start = vis.vector(x,0,0)+dy
    shape.starts = []
    for line in range(len(lines)):
        if align == 'left':
            x = 0
            ...
            x = -0.5*widths[line]
        shape.starts.append(vis.vector(x,line*vertical_spacing,0)+dy)
