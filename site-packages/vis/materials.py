...
tx_diffuse =  raw_texture( data=loadTGA(texturePath+"whiteDiffuseTexture"), interpolate=True)
tx_normal = raw_texture( data=loadTGA(texturePath+"normalMap"), interpolate=True)
tx_specular = raw_texture( data=loadTGA(texturePath+"specularMap"), interpolate=True)
    ...
    if mapping == "rectangular":
        ...
        return shader(
            ...
            shader = """
                ...
                void material_main(void) {
                    vec4 C;
                    if ( top_face > 0. ) C = texture2D( tex0, clamp(vec2(%f,%f) + vec2(%f,%f)*mat_pos.xz, vec2(%f,%f), vec2(%f,%f)) );
                    else {
                        if ( front_face > 0. ) C = texture2D( tex0, clamp(vec2(%f,%f) + vec2(%f,%f)*mat_pos.zy, vec2(%f,%f), vec2(%f,%f)) );
                        else {
                            if ( side_face > 0. ) C = texture2D( tex0, clamp(vec2(%f,%f) + vec2(%f,%f)*mat_pos.xy, vec2(%f,%f), vec2(%f,%f)) );
                            else
                                C = gl_Color;
                        }
                    }
                    material_color = lightAt( normalize(normal), normalize(-position),
                                                ...
                                                0. );
                    material_opacity = C.a;
                }""" % (adjust[0], adjust[1], adjust[2], adjust[3], adjust[4], adjust[5], adjust[6], adjust[7],
                        adjust[0], adjust[1], adjust[2], adjust[3], adjust[4], adjust[5], adjust[6], adjust[7],
                        adjust[0], adjust[1], adjust[2], adjust[2], adjust[4], adjust[5], adjust[6], adjust[7]))
...
[fragment]
    ...
    shader( name = "metal", version = 5.00, textures = [ tx_turb3 ], shader = """
        [fragment]
        uniform sampler3D tex0;
