...
class ModifiedInterpreter(InteractiveInterpreter):
    # TODO: there is still some flaky confusion here between the lifetime of the
    # subprocess and that of the RPC client in IDLE.  e.g. kill_subprocess() kills
    # both, but leaves the GC to close the listening socket, while unix_terminate()
    # kills just the subprocess.  At the very least, the names are confusing.
    ...
    def start_subprocess(self):
        ...
        try:
            self.rpcclt = MyRPCClient((LOCALHOST, 0))
        except socket.error, err:
            self.display_port_binding_error()
            return None

        # Find out what port we're listening on
...
class PyShell(OutputWindow):
    ...
    # Override menus
    # See macosxSupport.overrideMenu before changing this:
    menu_specs = [
        ("file", "_File"),
        ("edit", "_Edit"),
        ("debug", "_Debug"),
