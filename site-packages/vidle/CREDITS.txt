...
VIDLE is a new fork of IDLE created in December 2008 by VPython developers (David
Scherer and Bruce Sherwood).  We hope that our bug fixes and interface improvements
will someday again be integrated with the IDLE that ships with Python; in the meantime
we need to deliver them to our users.  VIDLE is based on IDLE 1.2.2.
VIDLE's source and version history beginning with a checkin of IDLE 1.2.2 can be found
in VPython's CVS repository (follow directions from www.vpython.org).
