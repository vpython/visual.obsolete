...
light::gl_begin( GLenum id, double gcf) const
{
	glEnable( id);
	if (attenuated()) {
		glLightf( id, GL_CONSTANT_ATTENUATION, (float)constant_attenuation);
	...
	if (spotlight()) {
		...
		if (spot_cutoff != 180) {
			...
			float spot_dir[] = { (float)_spot_direction.x, (float)_spot_direction.y, (float)_spot_direction.z, 1.0f };
